...
protected:
    ...
    {
        graph = new LGraph();
        auto node1 = graph->addBaseNode(QPoint(0, 10), "Node").toStrongRef();
        auto node2 = graph->addBaseNode(QPoint(100, 100), "Node").toStrongRef();
        node1_id = node1->ID();
        node2_id = node2->ID();
        ...
        ASSERT_TRUE(graph->addConnection(pin1_data, pin2_data)) << "Couldn't add connection";
...
TEST_F(Protocolization, PinDirectionCompatibility)
{
    ...
    EXPECT_EQ((protocol::PinDirection)dir, protocol::PinDirection::IN);
}
